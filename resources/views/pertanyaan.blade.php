...
@section('content')
    ...
    <div
        class="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-slate-50 to-teal-50 p-4 md:p-8 overflow-hidden font-sans relative">

        {{-- Decorative Blobs --}}
        <div
            class="absolute top-0 left-0 w-96 h-96 bg-teal-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob">
        </div>
        <div
            class="absolute top-0 right-0 w-96 h-96 bg-cyan-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000">
        </div>
        <div
            class="absolute -bottom-32 left-20 w-96 h-96 bg-emerald-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000">
        </div>

        {{-- MAIN CONTAINER --}}
        <div
            class="w-full max-w-6xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-[85vh] md:h-[80vh] relative z-10 border border-white/50">
            ...
            <div id="global-loading" class="absolute inset-0 z-50 bg-white flex flex-col items-center justify-center">
                <div class="relative">
                    ...
                    <div
                        class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-teal-600 text-xs font-bold">
                        CVS</div>
                </div>
            ...
            <div class="md:w-1/3 bg-teal-900 relative overflow-hidden flex flex-col justify-between p-8 text-white">
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-teal-200 text-sm md:text-base opacity-80">Jawablah dengan jujur sesuai kondisi mata Anda.
                    </p>
                </div>
                ...
                <div class="relative z-10 mt-auto">
                    ...
                    <div class="w-full bg-teal-800/50 rounded-full h-2 overflow-hidden backdrop-blur-sm">
                        <div id="progress-bar"
                            class="bg-gradient-to-r from-teal-400 to-emerald-400 h-full rounded-full transition-all duration-700 ease-out w-0">
                        </div>
                    </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-teal-800 flex items-center justify-between">
                        <div>
                            ...
                            <p id="question-counter-large" class="text-2xl font-bold">0<span
                                    class="text-teal-600 text-lg">/0</span></p>
                        </div>
            ...
            <div class="md:w-2/3 bg-white relative flex flex-col">
                ...
                <div class="flex-1 overflow-y-auto p-6 md:p-12 custom-scrollbar flex items-center">
                    <div class="w-full max-w-2xl mx-auto">
                        ...
                        <div id="question-container" class="transition-all duration-500">
                            <h2 id="q-text"
                                class="text-xl md:text-2xl font-semibold text-gray-800 leading-relaxed mb-8 animate-fade-in-up">
                            </h2>
                        ...
                        <div id="question-loading" class="hidden flex-col items-center justify-center py-12">
                            <div
                                class="w-10 h-10 border-4 border-gray-200 border-t-teal-500 rounded-full animate-spin mb-4">
                            </div>
                ...
                <div
                    class="border-t border-gray-100 p-4 md:p-6 bg-white/80 backdrop-blur flex justify-between items-center">
    ...
    <script>
        ...
        function renderCurrentQuestion() {
            ...
            if (qText) {
                ...
                // ✅ TAMBAHAN: Deteksi tipe pertanyaan dan tambahkan badge
                let questionHTML = '';
                ...
                if (currentCategory === 1) {
                    ...
                    if (q.type === 'inklusi') {
                        ...
                        <div class="flex items-start gap-3 mb-4">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700 border border-blue-300">
                                <i class="fas fa-check-circle mr-1.5"></i>
                                INKLUSI
                            </span>
                        </div>
                        <span class="text-xl md:text-2xl font-semibold text-gray-800 leading-relaxed">${q.question}</span>
                    `;
                    } else if (q.type === 'eksklusi') {
                        questionHTML = `
                        <div class="flex items-start gap-3 mb-4">
        ...
        async function nextCategory() {
            ...
            // Jika ini kategori 1 (Screening), kita wajib cek hasilnya dulu
            if (currentCategory === 1) {
                // Tambahkan variable untuk menampung hasil
                const screeningPassed = await batchSaveJawaban();
            ...
            // Jika lolos atau bukan kategori 1, baru lanjut
        ...
        async function batchSaveJawaban() {
            ...
            if (screening.length === 0 || !idPemeriksaan) {
                return true; // Tidak ada screening, lanjutkan
            }
            ...
            try {
                ...
                console.log('📤 Sending screening data:', mapS);
                ...
                console.log('📥 Screening response:', r);

                // ✅ CRITICAL FIX: Cek response dengan teliti
                if (r.complete === true && r.lolos === false) {
                    ...
                    return false; // ❌ Screening gagal
                }

                if (r.complete === true && r.lolos === true) {
                    console.log('✅ SCREENING PASSED');
                    return true; // ✅ Screening lolos
                }

                // Belum complete, return true untuk lanjut simpan
                console.log('⏳ Screening not complete yet');
                return true;
